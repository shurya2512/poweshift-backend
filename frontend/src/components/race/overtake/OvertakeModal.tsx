...
// Typographic parts shared with the report. The breakdown reads the same on both
// pages, so it is one component with one set of type rather than two that drift.
import { Aside, DataList, Datum, P, Subhead } from '../report/article';
...
/**
 * One decision window in full: the lines that were open, the one taken, and every
 * reason the model gives for taking it.
 *
 * The diagram is a schematic — the backend supplies the options, their costs and the
 * outcome, but not path geometry, and the caption under it says so.
 *
 * Rendered through a portal on the body. Several of the panels this opens from carry
 * `backdrop-filter`, which makes them the containing block for anything fixed inside
 * them — the overlay would be clipped to the panel rather than covering the page.
 */
export function OvertakeModal(props: {
  move: OvertakeMove;
