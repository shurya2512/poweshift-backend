...
export const LimelightNav = ({
  ...
  return (
    <nav className={`relative inline-flex items-center h-10 rounded-[14px] bg-white/[0.03] text-white border border-white/[0.05] px-1.5 ${className}`}>
          ...
          >
            {cloneElement(icon, {
              className: `w-4 h-4 transition-opacity duration-100 ease-in-out ${
                activeIndex === index ? 'opacity-100 text-blue-400' : 'opacity-40 hover:opacity-70'
      ...
      <div 
        ref={limelightRef}
        className={`absolute top-0 z-10 w-6 h-[2px] rounded-full bg-blue-500 shadow-[0_5px_15px_rgba(59,130,246,0.8)] ${
          isReady ? 'transition-[left] duration-500 ease-in-out' : ''
        } ${limelightClassName}`}
        style={{ left: '-999px' }}
      >
        <div className="absolute left-[-50%] top-[2px] w-[200%] h-8 [clip-path:polygon(15%_100%,35%_0,65%_0,85%_100%)] bg-gradient-to-b from-blue-500/20 to-transparent pointer-events-none" />
      </div>
    </nav>
  );
};
